...
Uint8List format255(double x, double y, double scalar, int signal) {
  ...
  dm1 = sqrt(pow(nDistX, 2) + pow(pDistY, 2)) - sqrt(2); //pytagoras voor motorsnelheid
  ...
  vals[1] = (dm1 * scalar).floor(); //doe snelheid maal constante en rond het af naar beneden
...
Uint8List ud255(double y, double scalar, {int signal = 0xF3}) { // WIP
  Uint8List vals = Uint8List(5);
  vals[0] = signal;
...
Uint8List lr255(double x, double scalar, {int signal = 0xF4}) { // WIP
  return Uint8List(1);
}
